...
RUN chmod +x "$CODE_DIR"/bin/*.sh \
    ...
    && TIMEOUT=600 gosu "$ARCHIVEBOX_USER" archivebox install archivewebpage liteparse mercury readability search_backend_sonic opendataloader search_backend_ripgrep 2>&1 | tee -a /VERSION.txt \
